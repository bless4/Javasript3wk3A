...
//handling err in xhr request

function createErrorPage(xhrRequest) {
  let root = document.getElementById("root");
  let errorDiv1 = createAndAppend("div",
   root, { id: "notfound" });
  let errorDiv2 = createAndAppend("div",
   errorDiv1, { id: "notfound1" });
  ...
  let errorMsg = createAndAppend("h2", errorDiv2, 
  { id: "errorMsg" });
  errorMsg.innerHTML = "Error: " + xhrRequest.statusText;
  console.log(xhrRequest.statusText);
}

// designing the page for DOM manipulation :
...
function createPage() {
  ...
  let leftHeader = createAndAppend("h3", leftSide,
   { text: "Repo Details" });
  ...
  let rightSide = createAndAppend("div", mainContainer,
   { class: "rightSide" });
  let rightHeading = createAndAppend("h3", rightSide, { text: 
    "Contributors" });
...
// Control select element and fill the data 
